...
def measure_lane_curvature_real(warped, left_fit, right_fit):
    '''
    Calculates the curvature of polynomial functions in meters.
    '''
    # Define conversions in x and y from pixels space to meters
    ...
    fit_cr = np.array([(left_fit_cr[i]+right_fit_cr[i])/2 for i in range(len(left_fit_cr))])
    curve_rad = (1 + (2 * fit_cr[0] * y_eval + fit_cr[1]) ** 2) ** (3 / 2) / abs(2 * fit_cr[0])
    return curve_rad
